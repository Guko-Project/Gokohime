...
插件支持 Omoi 的独立 Memory 配置和结构化事件 API。智能体记忆开关统一控制所有会话：开启后采集所有群和私聊，`enabled_groups` 只控制主动回复。插件每 30 秒刷新采集开关，服务端每次采集/提取检查实时设置；关闭立即停止新采集。无需插件侧额外开关。

事件使用真实 QQ 号、消息 ID、时间、原文和回复 ID，不从拼接后的 prompt 解析身份。普通群消息采集不依赖机器人回复，@ 处理器和私聊携带同样证据。服务端按事件 ID 去重；新版首次使用创建带来源绑定的新会话，不回填旧消息。

同一会话的创建/恢复有互斥保护；新的会话映射包含机器人及智能体 ID。同一 QQ 会话的完整分段回复串行发送，@ 回复第一段引用触发消息。现有 SPLIT、空行、SKIP 和延时规则继续生效。

记忆模型由 Omoi 的智能体设置选择，插件不持有另一套模型密钥。自动提取产生候选，需要在 Omoi 界面确认。网络采集失败只记录简要日志；缓冲中的事件可由后续对话重送，插件不提供进程重启后恢复的持久消息队列。
...
请求由 Omoi 按智能体的 `max_concurrent_requests` 调度（默认 3），同一会话逐条执行。插件不重复提交排队中的请求。`queue` 和 `ping` SSE 事件不会作为 QQ 文本发送；心跳用于保持排队期间的连接。流式对话不再有固定 180 秒总超时，90 秒无任何响应字节才取消连接；附件准备仍单独限制为 180 秒。模型执行超时由智能体配置控制，从开始执行时计时。


## 定时任务投递（2026-09）

- Omoi 保存和执行任务；QQ 插件通过通用 delivery API 绑定原会话，并轮询领取结果。后端不依赖 QQ API。
- 启动时开启投递消费者，以 `qq:<机器人账号>` 为实例；重连后无需等待新消息。
- 每次用户请求从 QQ Event 传递真实 actor_id；主动群聊携带 proactive=true，不能创建、修改或续期任务。
- 群任务仅创建者可通过对话管理。群会话重置会取消全部关联任务，因此重置限群主、管理员或机器人超级用户；私聊重置保持原行为。
- 最终回复仍使用配置的分隔符、空行后备、打字延迟及 skip_marker。
- 逐段保存发送进度及分段快照。已确认发送的段不重发；发送中断但无法确认是否成功时标记 unknown，人工检查，不自动重发。
- API Key 绑定不可迁移；更换机器人凭据后需要创建新会话。
- 自动测试使用模拟发送回调，禁止向真实 QQ 群发送测试消息。

记忆专用会话与原会话都支持已有定时任务投递；`.chat-reset` 撤销两种映射及其投递绑定。定时消息和即时回复共用同一会话发送锁，分段不会交错。

## 昵称触发（2026-09-16）

- 群消息正文任意位置包含 `bot.nickname`（如“鸽子姬”）时，进入与 @bot 相同的被动回复流程，不受主动发言概率、消息数或冷却间隔限制。
- 句中、句尾昵称保留在发送给 Omoi 的原文中；仅匹配文本，不匹配图片等附件的 URL 或元数据。空昵称不匹配，机器人自身消息不触发。
- 仍保留 `skip_prefixes` 命令过滤和现有 `[SKIP]` 静默行为；私聊行为不变。

## 增量上下文（2026-09-23）

- 群消息不再每次重发 20 条快照。缓冲区为每条消息分配单调序号并维护已发送游标：常规请求只携带 `seq > sentSeq` 的增量（`群「X」的新消息：`），Omoi 会话历史本身即连续流水；新会话（创建、重置、404 重建）才发送一次全缓冲种子（沿用原“最近聊天记录”格式）。
- `pending → send → markSent` 由每群 `sendMu` 串行化，mention 与主动触发不会携带重叠增量；发送失败不推进游标，下回合自然重发；缓冲淘汰先于发送时增量块标注“（中间省略 N 条更早消息）”。
- `【QQ 回复格式】` 不再随每条消息附加（原先每条 user message 一份并永久留在历史里）；SPLIT/[SKIP] 语义由 Agent 系统提示词与定时任务投递绑定承担。`withReplyFormat` 仅用于 delivery 绑定的 `reply_instruction`。
- 附件选择只看本次增量：历史附件已作为 content block 留在会话早前消息中，不再跨回合重复携带。
- 存量会话历史已按同一规则迁移：每会话内转录行去重、首条保留种子格式、后续改写为增量格式、剥离格式模板。

## @ 他人识别（2026-09-18）

- 即时对话、群历史和记忆事件均保留 OneBot `at` 消息段，按原位置显示为 `@群名片(QQ号)`，无名片时使用昵称；查询失败仍保留 `@QQ号`。`at all` 显示为 `@全体成员`。
- 同一消息中的重复目标只查一次名片，查询总预算 2 秒；不查询私聊中的群名片。仅用于触发的 @机器人仍省略，保留单独 @ 的应答行为。
- 命令前缀继续按纯文本判断，避免 @ 前缀绕过过滤。只有 @ 他人的消息也能进入历史和记忆。测试使用模拟 OneBot/Omoi API，不向真实 QQ 群发送测试消息。
