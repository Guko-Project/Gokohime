...
type AIConfig struct {
	...
	CompactPreserveCount    int     `yaml:"compact_preserve_count"`
	ReplyProbability        float64 `yaml:"reply_probability"`
	ProactiveGroupWhitelist []int64 `yaml:"proactive_group_whitelist"`
}
...
func Load(path string) (*Config, error) {
	...
	if key := os.Getenv("ANTHROPIC_API_KEY"); key != "" && cfg.AI.AnthropicAPIKey == "" {
		cfg.AI.AnthropicAPIKey = key
	}
	if key := os.Getenv("OPENAI_API_KEY"); key != "" && cfg.AI.OpenAIAPIKey == "" {
		cfg.AI.OpenAIAPIKey = key
	}
	if key := os.Getenv("WEATHER_API_KEY"); key != "" && cfg.Weather.APIKey == "" {
		cfg.Weather.APIKey = key
	}
	if key := os.Getenv("SEARCH_API_KEY"); key != "" && cfg.Search.APIKey == "" {
	...
	if key := os.Getenv("SIMPLE_GPT_API_KEY"); key != "" && cfg.AI.OpenAIAPIKey == "" {
		cfg.AI.OpenAIAPIKey = key
	}
	if model := os.Getenv("SIMPLE_GPT_MODEL"); model != "" && cfg.AI.OpenAIModel == "" {
		cfg.AI.OpenAIModel = model
	}
	if apiBase := os.Getenv("SIMPLE_GPT_API_BASE"); apiBase != "" && cfg.AI.OpenAIAPIBase == "" {
		cfg.AI.OpenAIAPIBase = apiBase
	}
	if len(cfg.AI.ProactiveGroupWhitelist) == 0 {
		cfg.AI.ProactiveGroupWhitelist = parseInt64ListEnv("SIMPLE_GPT_PROACTIVE_GROUP_WHITELIST")
	}
	if key := os.Getenv("SIMPLE_GPT_WEATHER_API_KEY"); key != "" && cfg.Weather.APIKey == "" {
		cfg.Weather.APIKey = key
	}
	if city := os.Getenv("SIMPLE_GPT_WEATHER_CITY"); city != "" && cfg.Weather.DefaultCity == "" {
		cfg.Weather.DefaultCity = city
	}
	if key := os.Getenv("SIMPLE_GPT_SEARCH_API_KEY"); key != "" && cfg.Search.APIKey == "" {
		cfg.Search.APIKey = key
	}
	...
	if cfg.Banana.APIKey == "" {
		cfg.Banana.APIKey = cfg.AI.OpenAIAPIKey
	}
	if cfg.Banana.APIBase == "" {
		cfg.Banana.APIBase = cfg.AI.OpenAIAPIBase
	}
	if cfg.Banana.APIMode == "" {
		cfg.Banana.APIMode = "openai"
	}
	if cfg.Banana.Model == "" {
		cfg.Banana.Model = cfg.AI.OpenAIModel
	}
	if cfg.Banana.TimeoutSec <= 0 {
